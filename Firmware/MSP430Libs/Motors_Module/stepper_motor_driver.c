...
/************ robot_go_forward() **************
the robot will go forward by different speed
speed: motor speed 5~50 rpm
***********************************************/
void robot_go_forward(uint8_t speed) {
	CYCLES_COUNT_R = 0;
	CYCLES_COUNT_L = 0;
	stepper_motor_direction_set(BOTH_MOTOR, DIRECTION_FORWARD);
	set_speed(BOTH_MOTOR, speed);
	timer_enable();
	return_message("M:RUNNING.");
}
...
void robot_go_backward(uint8_t speed) {
	CYCLES_COUNT_R = 0;
	CYCLES_COUNT_L = 0;
	stepper_motor_direction_set(BOTH_MOTOR, DIRECTION_BACKWARD);
	set_speed(BOTH_MOTOR, speed);
	timer_enable();
}

/******** linear_voltage_caluclate() **********
the help function for robot_go_round(). 
calculate the voltage for different wheels.
radius: the distance from center to wheel
turn_radius: the turning radius robot make
speed: motor speed 5~50 rpm
return speed
***********************************************/
uint8_t linear_voltage_caluclate(uint16_t radius, uint16_t turn_radius, uint8_t turn_speed) {
	int i;
	float result = 0.0; 
	float ratio = radius / turn_radius;
...
/************* robot_go_round() ***************
the robot will go in a circle
turn_radius: the turning radius robot make > 100
speed: motor speed 5~50 rpm
direction: 0:cw 1:ccw
***********************************************/
void robot_go_round(uint16_t turn_radius, uint8_t speed, uint8_t direction) {
	uint16_t wheel_to_center_distance = WHEEL_DIFFERENCE / 2;
	uint16_t outerRadius = turn_radius + wheel_to_center_distance;
	uint16_t innerRadius = turn_radius - wheel_to_center_distance;
	uint8_t outerSpeed = linear_voltage_caluclate(outerRadius, turn_radius, speed); 
	uint8_t innerSpeed = linear_voltage_caluclate(innerRadius, turn_radius, speed); ;

	switch(direction) {
		case DIRECTION_CLOCIWISE:
			robot_go(innerSpeed, DIRECTION_FORWARD, outerSpeed, DIRECTION_FORWARD);
			break;
		case DIRECTION_COUNTERCLOCKWISE:
			robot_go(outerSpeed, DIRECTION_FORWARD, innerSpeed, DIRECTION_FORWARD);
			break;
		default:
			break;
	}
